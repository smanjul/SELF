PROGRAM Advection_ConservativeForm_3D
...
USE SELF_CLI
  ...
  TYPE(Advection3D),TARGET :: semModel
  TYPE(MPILayer),TARGET :: decomp
  TYPE(CLI) :: args
  CHARACTER(LEN=SELF_EQUATION_LENGTH) :: initialCondition(1:nvar)
  CHARACTER(LEN=SELF_EQUATION_LENGTH) :: velocityField(1:3)
  CHARACTER(LEN=255) :: SELF_PREFIX
    ...
    CALL get_environment_variable("SELF_PREFIX", SELF_PREFIX)
    CALL mesh % Read_HOPr(TRIM(SELF_PREFIX)//"/etc/mesh/Sphere/sphere_mesh.h5")
    ...
    velocityField = (/"vx=1.0","vy=0.0","vz=0.0"/)
    CALL semModel % SetVelocityField( velocityField )

    ! Set the initial condition
    initialCondition = (/"s = 1.0"/)
...
END PROGRAM Advection_ConservativeForm_3D
